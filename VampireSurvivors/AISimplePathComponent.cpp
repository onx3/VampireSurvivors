...
{
		mPathRecalcTimer = static_cast<float>(rand() % 1000) / 1000.f * mPathRecalcInterval;
}
...
void AISimplePathComponent::Update(float deltaTime)
{
		...
		if (distance > 0.f)
		{
				...
				// Check tile walkability before moving
				auto * pLevelManager = GetGameManager().GetManager<LevelManager>();
				if (pLevelManager)
				{
						int tileX = static_cast<int>(nextPos.x / BD::gsPixelCountCellSize);
						int tileY = static_cast<int>(nextPos.y / BD::gsPixelCountCellSize);

						if (pLevelManager->IsTileWalkableAI(tileX, tileY))
						{
								myGameObj.SetPosition(nextPos);
						}
				}
		}
}
